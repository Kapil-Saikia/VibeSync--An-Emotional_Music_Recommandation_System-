...
def init_sqlite():
    ...
    # Check if is_admin column exists in existing database, if not add it
    ...
    if not admin_user:
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?)
        ''', ('Admin', 'User', admin_email, admin_hash, 1))
...
def login():
    ...
    try:
        ...
        # Find user (include is_admin column)
        cursor.execute('''
...
def home():
    ...
    # Both regular users and admins can access home page
...
@app.route('/manual_mood', methods=['POST'])
@login_required
def manual_mood():
    try:
        data = request.get_json()
        ...
        if not mood:
            return jsonify({'success': False, 'message': 'No mood'}), 400
        ...
        cursor.execute('''
            INSERT INTO emotion_history (user_id, email, emotion, confidence)
            VALUES (?, ?, ?, ?)
        ''', (session['user_id'], session['email'], mood, 100.0))
        conn.commit()
        conn.close()
        ...
        return jsonify({
            ...
            'confidence': 100.0,
            'songs': songs,
            'message': f'🎭 {mood.capitalize()} mood!',
            'isManual': True
        }), 200
...
if __name__ == '__main__':
    ...
    print("   Home:   http://localhost:5000/home (regular users)")
